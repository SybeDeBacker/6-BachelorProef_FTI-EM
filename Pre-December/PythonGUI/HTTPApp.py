...
class HTTPClientApp:
    def __init__(self, master):
        ...
        self.heartbeat_monitor = Thread(target=self.test_connection_heartbeat)
    ...
    def test_connection_heartbeat(self):
        ...
        # Reschedule
        Timer(interval=10, function=self.test_connection_heartbeat).start()
    ...
    def send_move_command(self):
        ...
        try:
            ...
            self.display_message(f"Error: Position values must be numbers {e}")
    ...
    def send_home_command(self):
        ...
        message = json.dumps(request)
        self.send_message(message,'home_robot')
        ...
        try:
            ...
            if self.loopback:
                ...
                try:
                    ...
                    self.connected = False
                    self.update_status(f"Connection failed: Loopback at 127.0.0.1 failed", "red")
                    return self.connected
            self.connected = False
            self.update_status(f"Connection failed: no loopback attempt was made.", "red")
            return False
    
    def reconnect(self):
        # Update status to indicate reconnection is in progress
        self.server_url = "http://10.0.1.250"
        Thread(target=self.connect_to_server).start()
        ...
        if self.connected:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.display_message(f"Failed to send: {message} \n\nServer:  {f'{response.status_code}' : >5} | {response.text}")
            except requests.exceptions.RequestException as e:
